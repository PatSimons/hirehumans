...
(() => {
  ...
  function getTargetElements(target) {
    ...
    return toArray4(elements).reduce((result, element) => {
      return [...result, ...toArray4(element).filter(isNode)];
    }, []);
  }
  var {
    entries,
  ...
  function set(owner, key, value) {
    ...
    const id = owner[expando2] || (owner[expando2] = ++uid);
  ...
  function get(owner, key) {
    const id = isObject(owner) ? owner[expando2] : null;
  ...
  function remove(element) {
    const id = element && element[expando2];
    if (id) {
      delete element[id];
      delete cache[id];
    }
  ...
  function splitWordsAndChars(textNode, settings) {
    settings = extend2(defaults3, settings);
  ...
  function split(node, settings) {
    ...
    const childNodes = toArray4(node.childNodes);
  ...
  function unSplitWords(element) {
    if (!get(element).isWord) {
      toArray4(element.children).forEach((child) => unSplitWords(child));
    } else {
      remove(element);
  ...
  var _defaults3 = extend2(defaults3, {});
  var SplitType = class {
    /**
    ...
    static set defaults(options) {
      _defaults3 = extend2(_defaults3, parseSettings(options));
    }
    /**
     * Sets the default settings for all SplitType instances.
    ...
    constructor(elements, options) {
      this.isSplit = false;
      this.settings = extend2(_defaults3, parseSettings(options));
    ...
    split(options) {
      ...
      if (options !== void 0) {
        this.settings = extend2(this.settings, parseSettings(options));
      }
  ...
  // src/global.ts
  gsapWithCSS.registerPlugin(ScrollTrigger2, Draggable, SplitType, sortable_esm_default);
  ...
  window.Webflow.push(() => {
    ...
    const sortableLists = document.querySelectorAll('[cs-el="sortable-list"]');
    ...
    mm.add(
      ...
      (context3) => {
        ...
        function init4() {
          ...
          const scrolltriggerOnEnterElms = gsapWithCSS.utils.toArray('[cs-st*="scroll-in"]');
        ...
        const st_paralaxBgElms = gsapWithCSS.utils.toArray('[cs-st*="paralax-bg"]');
