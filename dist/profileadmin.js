...
(() => {
  ...
  window.Webflow.push(() => {
    ...
    if (services) {
      const serviceItems = document.querySelectorAll('[cs-el="serviceItem"]');
      serviceItems.forEach((item) => {
        const serviceEdit = item.querySelector('[cs-el="serviceEdit"]');
        const serviceFormInputs = item.querySelector('[cs-el="serviceFormInputs"]');
        const serviceFormInputsHeight = serviceFormInputs?.clientHeight ?? 0;
        gsapWithCSS.set(serviceFormInputs, { opacity: 0, display: "none", height: 0 });
        const tl_editService = gsapWithCSS.timeline({ paused: true });
