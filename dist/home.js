...
(() => {
  ...
  window.Webflow ||= [];
  window.Webflow.push(() => {
    function sortListByNumber(list2) {
      const pattern = /_(\d+):/;
      list2.sort((a, b) => {
    ...
    function sumLastNumbers(sortedList2) {
      const sumMap2 = /* @__PURE__ */ new Map();
      sortedList2.forEach((item) => {
        const [name, numberStr] = item.split("_");
        const number = parseInt(numberStr.split(":")[1].trim());
        if (sumMap2.has(name)) {
          const currentSum = sumMap2.get(name) || 0;
          sumMap2.set(name, currentSum + number);
        } else {
          sumMap2.set(name, number);
        }
      });
      const sortedSumMap = new Map([...sumMap2.entries()].sort((a, b) => b[1] - a[1]));
      return sortedSumMap;
    }
    const list = [
      "Ruler_42: 1 ",
